...
public class TRControllerOriRenderer extends TRControllerAbstractRenderer {

    // TODO(Mine): Replace all goauld stuff here with ori
    @Override
    public void renderController(TRControllerAbstractTile te, TRControllerAbstractRendererState rendererState) {
        TRControllerOriRendererState rs = ((TRControllerOriRendererState) rendererState);
        if(rs == null) return;
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 2; x++) {
                ...
                ResourceLocation texture = ((TRControllerOriRendererState) rendererState).getButtonTexture(symbolId, rendererState.getBiomeOverlay());
