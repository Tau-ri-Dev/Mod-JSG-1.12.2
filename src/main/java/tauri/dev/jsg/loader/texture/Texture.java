...
public class Texture {
	...
	public Texture(BufferedImage bufferedImage, boolean desaturate) {
		...
		if (desaturate) {
			JSG.logger.info("Started desaturation of the event horizon texture");
			...
			JSG.logger.info("Finished desaturation of the event horizon texture");
...
//        	}
	...
	private static class RasterThread extends Thread {
		...
		@Override
		public void run() {
			JSG.logger.debug(String.format("Starting thread (%d, %d) to (%d, %d)", x, y, x+w, y+h));
			...
			JSG.logger.debug(String.format("Finished thread (%d, %d) to (%d, %d)", x, y, x+w, y+h));
		}
	}
}