package tauri.dev.jsg.gui.mainmenu.screens.options;

import tauri.dev.jsg.JSG;
import tauri.dev.jsg.config.JSGConfig;
import tauri.dev.jsg.gui.base.JSGGuiButton;
import tauri.dev.jsg.gui.base.JSGGuiSlider;
import tauri.dev.jsg.gui.base.JSGOptionButton;
import tauri.dev.jsg.gui.mainmenu.screens.bindings.JSGGuiKeyBindingList;
...
public class JSGBindingOptions extends GuiControls {
    public JSGBindingOptions(GuiScreen screen, GameSettings settings, BiomeOverlayEnum overlay) {
    ...
    protected java.util.List<GuiButton> jsgButtonSliders = new ArrayList<>();
    ...
    public long time;
    private JSGGuiKeyBindingList keyBindingList;
    private JSGGuiButton buttonReset;
    private int eventButton;
    private long lastMouseEvent;
    private int touchValue;
    public JSGOptionButton autoJump_button;
    ...
    public void initGui(){
        ...
        jsgButtonList.clear();
        this.keyBindingList = new JSGGuiKeyBindingList(this, this.mc);
        this.jsgButtonList.add(new JSGGuiButton(200, width / 2 + 5, height - 29, 150, 20, I18n.format("gui.done")));
        this.buttonReset = this.addButton(new JSGGuiButton(201, width / 2 - 205, height - 29, 150, 20, I18n.format("controls.resetAll")));
        ...
        for (GameSettings.Options gamesettings$options : OPTIONS_ARR)
        {
            ...
            {
                this.jsgButtonList.add(new JSGGuiSlider(gamesettings$options.getOrdinal(), this.width / 2 - 205 + i % 2 * 210, 18 + 24 * (i >> 1), gamesettings$options));
            }
            else
            {
                if(gamesettings$options != GameSettings.Options.AUTO_JUMP)
                    this.jsgButtonList.add(new JSGOptionButton(gamesettings$options.getOrdinal(), this.width / 2 - 205 + i % 2 * 210, 18 + 24 * (i >> 1), gamesettings$options, this.options.getKeyBinding(gamesettings$options)));
                else {
                    autoJump_button = new JSGOptionButton(gamesettings$options.getOrdinal(), this.width / 2 - 205 + i % 2 * 210, 18 + 24 * (i >> 1), gamesettings$options, this.options.getKeyBinding(gamesettings$options));
                    autoJump_button.enabled = false;
                    this.jsgButtonList.add(autoJump_button);
                }
            }
