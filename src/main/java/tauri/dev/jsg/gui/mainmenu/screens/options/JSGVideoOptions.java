...
public class JSGVideoOptions extends GuiVideoSettings {
    ...
    protected static final ResourceLocation BACKGROUND_TEXTURE =
            tauri.dev.jsg.config.JSGConfig.mainMenuConfig.disableJSGMainMenu ? null :
                    new ResourceLocation(JSG.MOD_ID, "textures/gui/mainmenu/background.jpg");
    ...
    public JSGVideoOptions(GuiScreen parentScreenIn, GameSettings gameSettingsIn, BiomeOverlayEnum overlay) {
    ...
    public void updateRingSpeed(){
        if(!JSGConfig.mainMenuConfig.gateRotation) return;
        if (!speedUpGate) {
    ...
    public void initGui(){
        ...
        jsgButtonList.add(new JSGGuiButton(200, this.width / 2 - 100, this.height - 27, I18n.format("gui.done")));
        ...
        for (GameSettings.Options gamesettings$options : VIDEO_OPTIONS) {
            ...
            if (gamesettings$options.isFloat())
                jsgButtonSliders.add(new JSGGuiSlider(gamesettings$options.getOrdinal(), x, y, gamesettings$options));
            else
                jsgButtonList.add(new JSGOptionButton(gamesettings$options.getOrdinal() + 400, x, y, gamesettings$options, Minecraft.getMinecraft().gameSettings.getKeyBinding(gamesettings$options)));
            i++;
        }
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.enabled){
            ...
            if(button.id >= 400){
                this.mc.gameSettings.setOptionValue(((JSGOptionButton) button).getOption(), 1);
                button.displayString = this.mc.gameSettings.getKeyBinding(((JSGOptionButton) button).getOption());
                this.mc.gameSettings.saveOptions();
            }
        }
    }
