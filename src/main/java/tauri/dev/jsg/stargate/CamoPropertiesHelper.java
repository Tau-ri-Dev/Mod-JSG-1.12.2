...
public class CamoPropertiesHelper {
	...
	public static AxisAlignedBB getStargateBlockBoundingBox(IBlockState state, IBlockAccess access, BlockPos pos, boolean collision) {
		...
		if (!state.getValue(JSGProps.RENDER_BLOCK)) {
			switch(state.getValue(JSGProps.FACING_HORIZONTAL).getAxis()) {
				case X:
			...
			if (state.getBlock() == (Block) JSGBlocks.STARGATE_MILKY_WAY_MEMBER_BLOCK && access.getBlockState(pos).getBlock() == (Block) JSGBlocks.STARGATE_MILKY_WAY_MEMBER_BLOCK) {
				if (!(state instanceof ExtendedBlockState))
					state = state.getBlock().getExtendedState(state, access, pos);

				IBlockState camoState = ((IExtendedBlockState) state).getValue(JSGProps.CAMO_BLOCKSTATE);
