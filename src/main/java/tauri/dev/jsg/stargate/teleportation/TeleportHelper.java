...
public class TeleportHelper {
    ...
    public static void teleportEntity(Entity entity, BlockPos sourceGatePos, StargatePos targetGatePos, float rotation, Vector2f motionVector) {
        ...
        if (sourceDim == targetGatePos.dimensionID) {
            ...
            if (!fireTravelToDimEvent(entity, targetGatePos.dimensionID)) return;
    ...
    public static void teleportEntityToStargate(Entity entity, StargatePos targetGatePos, boolean bypassCancelEvent) {
        ...
        BlockPos tPos = targetTile.getGateCenterPos();
        Vec3d pos = new Vec3d(tPos.getX(), tPos.getY(), tPos.getZ());

        boolean isPlayer = entity instanceof EntityPlayerMP;

        if (sourceDim == targetGatePos.dimensionID) {
            setRotationAndPosition(entity, entity.rotationYaw, pos);
        } else {
            if (!fireTravelToDimEvent(entity, targetGatePos.dimensionID) && !bypassCancelEvent) return;

            final Vec3d posFinal = pos;

            ITeleporter teleport = (world1, entity1, yaw) -> setRotationAndPosition(entity1, entity1.rotationYaw, posFinal);

            if (isPlayer) {
                EntityPlayerMP player = (EntityPlayerMP) entity;
                Objects.requireNonNull(player.getServer()).getPlayerList().transferPlayerToDimension(player, targetGatePos.dimensionID, teleport);
            } else {
                entity.changeDimension(targetGatePos.dimensionID, teleport);
            }
        }
    }

    //@SuppressWarnings("all")
    public static boolean fireTravelToDimEvent(Entity entity, int dimension) {
        PortalEvent.ENTITIES_IGNORED.put(entity.getEntityId(), entity);
