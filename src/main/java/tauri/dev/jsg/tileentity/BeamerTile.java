...
//						JSG.info(String.format("power=%d, fluids=%s, items=%s", powerTransferredSinceLastSignal, fluidsTransferredSinceLastSignal.toString(), itemsTransferredSinceLastSignal.toString()));
			...
			if (lastBeamerStatus != beamerStatus) {
				if(isLinked() && targetBeamerWorld != null && targetBeamerPos != null && !(getLinkedGateTile().getStargateState().idle()) && !(getLinkedGateTile().getStargateState().incoming())){
					BeamerTile targetTile = (BeamerTile) targetBeamerWorld.getTileEntity(targetBeamerPos);
					if(targetTile != null){
						StargateAbstractBaseTile targetGate = targetTile.getLinkedGateTile();
						if(targetGate != null){
							updateTargetBeamerData(targetGate.getNetwork().getStargate(targetGate.getStargateAddress(SymbolTypeEnum.MILKYWAY)));
						}
						else targetBeamerPos = null;
					}
					else targetBeamerPos = null;
				}
				syncToClient();
	...
	public void updateTargetBeamerData(StargatePos targetGatePos){
	...
	private BlockPos findTargetBeamerPos(StargatePos targetGatePos) {
		...
		EnumFacing targetFacing = targetWorld.getBlockState(targetGatePos.gatePos).getValue(JSGProps.FACING_HORIZONTAL);
		...
		do{
			loop++;
			targetPos = getNearest(world, targetGatePos.gatePos, blacklist);
			if(targetPos == null)
				break;


			BeamerTile targetBeamer = (BeamerTile) world.getTileEntity(targetPos);
		...
		for (int i = 1; i<= tauri.dev.jsg.config.JSGConfig.beamerConfig.reach; i++) {
...
//					targetBeamerTile.updateTargetBeamerPos(sourceGatePos, targetGatePos); // Intentionally swaped
	...
	private class ItemStackHandlerBeamer extends ItemStackHandler {
		...
		public boolean isItemValid(int slot, ItemStack stack) {
			...
			switch (slot) {
				case 0:
					return item == JSGItems.BEAMER_CRYSTAL_POWER || item == JSGItems.BEAMER_CRYSTAL_FLUID || item == JSGItems.BEAMER_CRYSTAL_ITEMS;
	...
	private StargateAbstractEnergyStorage energyStorage = new StargateAbstractEnergyStorage(tauri.dev.jsg.config.JSGConfig.beamerConfig.energyCapacity, tauri.dev.jsg.config.JSGConfig.beamerConfig.energyTransfer) {
	...
	public State getState(StateTypeEnum stateType) {
		switch (stateType) {
			...
			case RENDERER_STATE:
				...
				return new BeamerRendererState(beamerMode, beamerRole, beamerStatus, isObstructed, distance, beamOffsetFromTargetXClient, beamOffsetFromTargetYClient);
...
//			JSG.info("sending signal " + name + ", params: " + params[0].toString());
	...
	public Object[] getBufferStored(Context context, Arguments args) {
		...
		case FLUID: return new Object[] { fluidHandler.getFluidAmount(), (fluidHandler.getFluid() != null ? JSG.proxy.localize(fluidHandler.getFluid().getFluid().getUnlocalizedName()) : null) };
		case ITEMS:
