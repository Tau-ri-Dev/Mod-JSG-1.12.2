...
public class DHDPegasusTile extends DHDAbstractTile {
    ...
    public void activateSymbol(SymbolInterface symbolInt) {
        ...
        if (!gateTile.getStargateState().dialingComputer() || ((StargateClassicBaseTile) gateTile).getConfig().getOption(StargateClassicBaseTile.ConfigOptions.ENABLE_DHD_PRESS_SOUND.id).getBooleanValue()) {
            ...
            if (symbol.brb()) JSGSoundHelper.playSoundEvent(world, pos, SoundEventEnum.DHD_PEGASUS_PRESS_BRB);
            else JSGSoundHelper.playSoundEvent(world, pos, SoundEventEnum.DHD_PEGASUS_PRESS);
        }

        world.notifyNeighborsOfStateChange(pos, JSGBlocks.DHD_PEGASUS_BLOCK, true);
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("inventory")) {
            ...
            if (tagList.tagCount() > 0) {
                itemStackHandler.setStackInSlot(0, new ItemStack(JSGItems.CRYSTAL_CONTROL_PEGASUS_DHD));
