...
public class DHDPegasusTile extends TileEntity implements ILinkable, IUpgradable, StateProviderInterface, ITickable {
  ...
  public void activateSymbol(SymbolPegasusEnum symbol) {
    ...
    sendState(StateTypeEnum.DHD_ACTIVATE_BUTTON, new DHDActivateButtonPegasusState(symbol));
  }

  public void clearSymbols() {
    world.notifyNeighborsOfStateChange(pos, AunisBlocks.DHD_PEGASUS_BLOCK, true);

    sendState(StateTypeEnum.DHD_ACTIVATE_BUTTON, new DHDActivateButtonPegasusState(true));
  }
  ...
  public void setState(StateTypeEnum stateType, State state) {
    ...
    switch (stateType) {
      case RENDERER_STATE:
        horizontalRotation = world.getBlockState(pos).getValue(AunisProps.ROTATION_HORIZONTAL) * -22.5f;
        ...
        break;
      case DHD_ACTIVATE_BUTTON:
        DHDActivateButtonPegasusState activateState = (DHDActivateButtonPegasusState) state;

        if (activateState.clearAll) getRendererStateClient().clearSymbols(world.getTotalWorldTime());
        else getRendererStateClient().activateSymbol(world.getTotalWorldTime(), activateState.symbol);
