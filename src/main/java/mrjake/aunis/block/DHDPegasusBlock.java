...
public class DHDPegasusBlock extends Block {
  ...
  public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
    ...
    if (!world.isRemote) {
      ...
      DHDPegasusTile dhdTile = (DHDPegasusTile) world.getTileEntity(pos);
    ...
    if (!world.isRemote) {
      ...
      if (!player.isSneaking() && frontActivation) {
        ...
        if (!FluidUtil.interactWithFluidHandler(player, hand, world, pos, null)) {
          DHDPegasusTile tile = (DHDPegasusTile) world.getTileEntity(pos);
          if (!tile.tryInsertUpgrade(player, hand)) {
            player.openGui(Aunis.instance, GuiIdEnum.GUI_PEGASUS_DHD.id, world, pos.getX(), pos.getY(), pos.getZ());
          }
        }
      }
    }
