...
public abstract class StargateClassicMemberBlock extends StargateAbstractMemberBlock {
    ...
    public StargateClassicMemberBlock(String blockName) {
        ...
        setDefaultState(blockState.getBaseState().withProperty(AunisProps.FACING_HORIZONTAL, EnumFacing.NORTH).withProperty(AunisProps.MEMBER_VARIANT, EnumMemberVariant.RING).withProperty(AunisProps.RENDER_BLOCK, true));
    }
    ...
    public int getMetaFromState(IBlockState state) {
        return (state.getValue(AunisProps.MEMBER_VARIANT).id << 3) | (state.getValue(AunisProps.RENDER_BLOCK) ? 0x04 : 0) | state.getValue(AunisProps.FACING_HORIZONTAL).getHorizontalIndex();
    }

    @Override
    public IBlockState getStateFromMeta(int meta) {
        return getDefaultState().withProperty(AunisProps.MEMBER_VARIANT, EnumMemberVariant.byId((meta >> 3) & 0x01)).withProperty(AunisProps.RENDER_BLOCK, (meta & 0x04) != 0).withProperty(AunisProps.FACING_HORIZONTAL, EnumFacing.getHorizontal(meta & 0x03));
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            ...
            return heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_MILKY_WAY_MEMBER_BLOCK) && heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_MILKY_WAY_BASE_BLOCK) && heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_UNIVERSE_BASE_BLOCK) && heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_UNIVERSE_MEMBER_BLOCK) && heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_PEGASUS_BASE_BLOCK) && heldItem != Item.getItemFromBlock(AunisBlocks.STARGATE_PEGASUS_MEMBER_BLOCK);
        }
    }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote) {
            ...
            state = state.withProperty(AunisProps.MEMBER_VARIANT, EnumMemberVariant.byId((stack.getMetadata() >> 3) & 0x01)).withProperty(AunisProps.RENDER_BLOCK, true).withProperty(AunisProps.FACING_HORIZONTAL, facing);
