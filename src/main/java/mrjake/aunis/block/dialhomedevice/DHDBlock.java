...
public class DHDBlock extends Block {
  ...
  public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
    ...
    if (!world.isRemote) {
      ...
      DHDMilkyWayTile dhdMilkyWayTile = (DHDMilkyWayTile) world.getTileEntity(pos);
      dhdMilkyWayTile.updateLinkStatus(world, pos);
    }
  }
    ...
    if (!world.isRemote) {
      ...
      if (!player.isSneaking() && backActivation) {
        ...
        if (!FluidUtil.interactWithFluidHandler(player, hand, world, pos, null)) {
          DHDMilkyWayTile tile = (DHDMilkyWayTile) world.getTileEntity(pos);
  ...
  public void breakBlock(World world, BlockPos pos, IBlockState state) {
    ...
    if (!world.isRemote) {
      ...
      ItemHandlerHelper.dropInventoryItems(world, pos, dhdMilkyWayTile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null));
    }
  ...
  private int getPower(IBlockAccess world, BlockPos pos) {
    DHDMilkyWayTile dhdMilkyWayTile = (DHDMilkyWayTile) world.getTileEntity(pos);
    if (!dhdMilkyWayTile.isLinked()) return 0;
